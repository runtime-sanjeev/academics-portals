...
class DashboardController extends Controller
{
    public function studentCounts(Request $request)
    {
        ...
        // Get processed and unprocessed student counts by category
        ...
        foreach ($processedCounts as $category => $counts) {
            foreach ($counts as $count) {
                $status = ($count->processed === 'Y') ? 'processed' : 'unprocessed';
                $studentCounts['processed'][strtoupper($category)][$status] = $count->count;
            }
        }
